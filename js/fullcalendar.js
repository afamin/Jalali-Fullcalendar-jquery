...
// (such as highlight rows, day rows, helper rows, etc).
	...
	$.extend(Grid.prototype, {
		...
		headCellHtml: function(row, col, date) {
			...
			var patt = /[0-9]*[1-9][/][0-9]*[1-9]/g;
			if(htmlEscape(calendar.formatDate(date, colFormat)).match(patt))
			{
				var st1 =moment(date.format('YYYY/M/D'),'YYYY/M/D').format('jM/jD');
				var st2=date.format('ddd')


				headNOA =prN(st1)+''+st2;
			}else{
				headNOA =htmlEscape(calendar.formatDate(date, colFormat))
			};
		...
		getDayClasses: function(date) {
			...
			var da =moment(date.format('YYYY/M/D'),'YYYY/M/D').format('jM');
			var vi=moment(view.intervalStart.format('YYYY/M/D'),'YYYY/M/D').format('jM');
			if (
					view.name === 'month' &&
					da!= vi
			) {
...
// It is responsible for managing width/height.
	...
	$.extend(BasicView.prototype, {
		...
		numberCellHtml: function(row, col, date) {
			...
			var amin ='\''+ date.format()+'\'';
			var dayj =  moment(amin, 'YYYY-M-D').format('jYYYY/jM/jD');
			dayj =dayj.split("/")[2];
			return '' +
	...
	$.extend(MonthView.prototype, {
		...
		render: function(date) {
			...
			this.intervalStart = date.clone().stripTime().startOf('jMonth');
			this.intervalEnd = this.intervalStart.clone().add(1, 'jMonth');
